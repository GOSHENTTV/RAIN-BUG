...
{
	float3 res = TextureColor.Sample(Sampler1, f.txcoord0.xy).rgb;
	if (f.txcoord1.z)
	{
		float2 uv = f.txcoord0.xy;
		float time = f.txcoord1.w;

		// Generate a more complex noise pattern for the rain
		float2 noise_uv = uv * float2(2.0, 4.0) + float2(0.0, time * 0.2);
		float noise1 = frac(sin(dot(noise_uv, float2(12.9898, 78.233))) * 43758.5453);
		float2 noise_uv2 = uv * float2(1.5, 3.0) + float2(time * 0.1, 0.0);
		float noise2 = frac(sin(dot(noise_uv2, float2(34.345, 98.765))) * 23456.789);
		float noise = noise1 * noise2;

		// Create a ripple effect
		float2 ripple_uv = uv + float2(sin(uv.y * 10.0 + time * 2.0) * 0.01, cos(uv.x * 10.0 + time * 2.0) * 0.01);
		float ripple = TextureColor.Sample(Sampler1, ripple_uv).r;

		// Combine noise and ripple to create the rain effect
		float rain = saturate(noise + ripple);

		// Add distortion
		float2 distortion = float2(rain - 0.5, rain - 0.5) * 0.02;
		uv += distortion;

		// Sample the texture with the distorted UVs
		float3 distorted_color = TextureColor.Sample(Sampler1, uv).rgb;

		// Blend the rain effect with the original image
		float rain_intensity = ri;
		if (rd_racing)
		{
			rain_intensity *= rd_racing_intensity;
		}
		res = lerp(res, distorted_color, rain * rain_intensity * f.txcoord1.y);
	}
